...
2. On a machine with Tosca Commander 24, build the extension and copy the output over the committed
   ...
   is no second DLL to ship. The referenced Tricentis assemblies are not copied to the output folder
   (see the `ItemDefinitionGroup` in the csproj), so `bin\Release\net8.0\` holds only our DLL.
