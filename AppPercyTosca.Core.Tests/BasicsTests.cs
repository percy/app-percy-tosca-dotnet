...
namespace AppPercyTosca.Core.Tests
{
    ...
    {
        // Shapes standing in for what may or may not be in a Tosca process.
        private class ScriptableDriver
        {
            public static ScriptableDriver Current => new ScriptableDriver();
            public string SessionId => "s";
            public object? ExecuteScript(string script, object[] args) => null;
            public object GetScreenshot() => new object();
        }

        private class ScreenshotOnlyDriver
        {
            public object GetScreenshot() => new object();
        }

        private class NotADriver
        {
            public string Name => "x";
        }

        [Fact]
        public void AScriptableDriverOutranksAScreenshotOnlyOne()
        {
            // Weighted this way on purpose: a screenshot is already obtainable over HTTP, whereas
            // scripting is the capability that is missing entirely and unlocks full page capture.
            List<DriverProbe.Candidate> found = DriverProbe.Find(
                new[] { typeof(ScreenshotOnlyDriver), typeof(ScriptableDriver) });

            Assert.Equal(2, found.Count);
            Assert.Contains("ScriptableDriver", found[0].TypeName);
            Assert.True(found[0].CanExecuteScript);
            Assert.Equal("Current", found[0].StaticAccessor);
        }

        [Fact]
        public void TypesWithNeitherCapabilityAreNotCandidates()
        {
            Assert.Empty(DriverProbe.Find(new[] { typeof(NotADriver), typeof(string) }));
        }

        [Fact]
        public void AStaticAccessorIsOnlyReportedWhenItYieldsThisType()
        {
            // "Such a type exists" and "an instance can be reached from an extension" are different
            // findings, and only the second is actionable.
            Assert.Null(DriverProbe.Find(new[] { typeof(ScreenshotOnlyDriver) })[0].StaticAccessor);
        }

        [Fact]
        public void NothingFoundIsStatedAsSuchRatherThanLeftBlank()
        {
            string described = DriverProbe.Describe(new[] { typeof(NotADriver) });

            Assert.Contains("no driver object to borrow", described);
            Assert.Contains("HTTP", described);
        }

        [Fact]
        public void CandidatesWithoutScriptingAreCalledOutAsInsufficient()
        {
            string described = DriverProbe.Describe(new[] { typeof(ScreenshotOnlyDriver) });

            Assert.Contains("none can execute scripts", described);
            Assert.Contains("full page", described);
        }

        [Fact]
        public void AScriptableCandidateIsCalledOutAsWorthPursuing()
        {
            string described = DriverProbe.Describe(new[] { typeof(ScriptableDriver) });

            Assert.Contains("can execute scripts", described);
            Assert.Contains("Worth pursuing", described);
            Assert.Contains("ExecuteScript", described);
        }

        [Fact]
        public void ATypeThatCannotBeReflectedOverIsSkippedAndTheScanContinues()
        {
            // One unloadable type among thousands is the normal case when scanning a host process, so
            // it must not take the whole scan down.
            Func<Type, System.Reflection.MethodInfo[]> original = DriverProbe.MethodsOf;
            try
            {
                DriverProbe.MethodsOf = type => type == typeof(NotADriver)
                    ? throw new TypeLoadException("dependency missing")
                    : type.GetMethods();

                List<DriverProbe.Candidate> found = DriverProbe.Find(
                    new[] { typeof(NotADriver), typeof(ScriptableDriver) });
            ...
            {
                DriverProbe.MethodsOf = original;
            }
        }

        [Fact]
        public void TheListIsCappedSoTheDumpStaysReadable()
        {
            IEnumerable<Type> many = Enumerable.Repeat(typeof(ScriptableDriver), 20);

            Assert.Equal(3, DriverProbe.Describe(many, limit: 2).Split('\n').Length);
        }
    }
