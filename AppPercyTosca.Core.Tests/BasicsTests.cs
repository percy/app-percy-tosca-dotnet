...
namespace AppPercyTosca.Core.Tests
{
    ...
    public class PngSizeTests : CoreTestBase
    {
        private static byte[] Header(int width, int height, bool validSignature = true)
        {
            byte[] bytes = new byte[24];
            byte[] signature = { 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A };
            signature.CopyTo(bytes, 0);
            if (!validSignature) bytes[3] = 0x00;
        ...
        [Fact]
        public void DimensionsAreReadFromTheHeader()
        {
            Assert.Equal((1080, 2340), PngSize.TryRead(Header(1080, 2340)));
        }

        [Fact]
        public void ALargeDimensionIsReadAsBigEndian()
        {
            // A tablet in landscape exceeds a byte in both axes, so byte order is load-bearing rather
            // than incidental.
            Assert.Equal((2732, 2048), PngSize.TryRead(Header(2732, 2048)));
        }

        [Fact]
        public void TheRealSampleScreenshotReadsAsOneByOne()
        {
            // Guards the offsets against a real PNG rather than one this test built.
            Assert.Equal((1, 1),
                PngSize.TryRead(Convert.FromBase64String(StubMobileDriver.ValidPngBase64)));
        }

        [Fact]
        public void SomethingThatIsNotAPngIsRejected()
        {
            Assert.Null(PngSize.TryRead(Header(10, 10, validSignature: false)));
        }

        [Theory]
        [InlineData(0, 100)]
        [InlineData(100, 0)]
        public void AZeroDimensionIsRejectedBecauseItIsNotUsable(int width, int height)
        {
            Assert.Null(PngSize.TryRead(Header(width, height)));
        }

        [Fact]
        public void TooFewBytesOrNoneAtAllAreRejected()
        {
            Assert.Null(PngSize.TryRead(null));
            Assert.Null(PngSize.TryRead(new byte[10]));
        }

        [Fact]
        public void AFileIsReadFromDisk()
        {
            string path = Path.Combine(Path.GetTempPath(), "percy-png-" + Guid.NewGuid() + ".png");
            try
            {
                File.WriteAllBytes(path, Header(828, 1792));
                Assert.Equal((828, 1792), PngSize.TryReadFile(path));
            }
            finally
            {
                File.Delete(path);
        ...
        public void AFileShorterThanTheHeaderIsRejected()
        {
            ...
            try
            {
                ...
                Assert.Null(PngSize.TryReadFile(path));
            }
            finally
            {
                File.Delete(path);
        ...
        [InlineData("")]
        [InlineData("   ")]
        public void NoPathIsRejected(string? path)
        {
            Assert.Null(PngSize.TryReadFile(path));
        }

        [Fact]
        public void AMissingFileIsReportedAsUnreadableRatherThanThrowing()
        {
            // A failure here should cost the tag its dimensions, not fail the snapshot.
            SetEnv("PERCY_LOGLEVEL", "debug");

            Assert.Null(PngSize.TryReadFile(
                Path.Combine(Path.GetTempPath(), "percy-does-not-exist-" + Guid.NewGuid() + ".png")));
            Assert.True(Logged("Could not read the screenshot dimensions"));
        }
    }
